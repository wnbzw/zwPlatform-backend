...
public class QuestionServiceImpl extends ServiceImpl<QuestionMapper, Question> implements QuestionService {
    ...
    public void batchDeleteQuestions(List<Long> questionIdList) {
        ...
        for(Long questionId:questionIdList){
            ...
            if(CollUtil.isNotEmpty(list)){
                List<Long> Ids = list.stream().map(QuestionBankQuestion::getId).collect(Collectors.toList());
                boolean b = questionBankQuestionService.removeBatchByIds(Ids);
                if(!b){
                    throw new BusinessException(ErrorCode.OPERATION_ERROR,"移除题库题目关系失败");
                }
            }
        }
        //2.再删除题目
        boolean b = this.removeBatchByIds(questionIdList);
        if(!b){
    ...
    public Page<Question> searchFromEs(QuestionQueryRequest questionQueryRequest) {
        ...
        if (StringUtils.isNotBlank(sortField)) {
            ...
            sortBuilder = SortBuilders.fieldSort(validSortField);
